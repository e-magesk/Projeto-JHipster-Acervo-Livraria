...
import { EntityArrayResponseType, PosicaoService } from '../service/posicao.service';
import { PosicaoDeleteDialogComponent } from '../delete/posicao-delete-dialog.component';
...
export class PosicaoComponent implements OnInit {
  ...
  protected readonly posicaoService = inject(PosicaoService);
  ...
  trackId = (item: IPosicao): number => this.posicaoService.getPosicaoIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.posicaos.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IPosicao[] | null): IPosicao[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const posicaosNew = this.posicaos();
      if (data) {
        for (const d of data) {
          if (posicaosNew.some(op => op.id === d.id)) {
            posicaosNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.posicaoService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
